      ...
      <div class="table-header">
        ...
        <div class="table-actions">
          <v-menu offset-y>
            ...
            <v-list dense>
              <v-list-item @click="exportEnvases">
                <v-list-item-title>Exportar</v-list-item-title>
              </v-list-item>
              <v-list-item @click="openImportEnvases">
  ...
  methods: {
    ...
    async exportEnvases() {
      await exportRowsToExcel({
        filename: 'envases',
        sheetName: 'Envases',
        rows: this.filteredEnvases,
        columns: [
          { label: 'Codigo', value: envase => envase.codigo },
          { label: 'Nombre', value: envase => envase.nombre },
          { label: 'Descripcion', value: envase => envase.descripcion },
          { label: 'Estado', value: envase => envase.estado ? 'Activo' : 'Inactivo' },
          { label: 'Fecha creacion', value: envase => this.formatDate(envase.fechaCreacion) }
        ]
      })
    },
    openImportEnvases() {
      this.$refs.envasesExcelInput.click()
    },
    async importEnvases(event) {
      const file = event.target.files[0]
      event.target.value = ''
      if (!file) return

      try {
        const result = await readRowsFromExcelFile(file, ENVASE_EXCEL_COLUMNS)
        ...
        for (const row of result.rows) {
          const envase = await this.$firebaseApi.create('envases', {
            codigo: row.Codigo,
            nombre: row.Nombre,
