    ...
    <v-dialog v-model="dialog" max-width="680" persistent>
      <v-card>
        ...
        <v-card-text>
          ...
          <v-autocomplete v-model="form.compartidoConId" :readonly="readOnly" :items="personalDisponible" item-text="nombres" item-value="id" label="Compartir con" outlined clearable hint="La persona podrá visualizar y mover la tarea" persistent-hint @change="syncSharedPerson" />
        </v-card-text>
...
import { createEmptyTareaForm, normalizeTarea, toTareaPayload, ESTADOS_TAREA, PRIORIDADES_TAREA } from '~/models/tarea'
...
export default {
  ...
  data() {
    return {
      ...
      kanbanColumns: {}, dialog: false, editingId: null, readOnly: false, form: createEmptyTareaForm(),
      headers: [
  ...
  computed: {
    ...
    tareasVisibles() {
      ...
      const correo = String(this.currentUser.correo || '').toLowerCase()
      return this.tareas.filter(tarea => tarea.creadorId === id || tarea.compartidoConId === id || (correo && tarea.compartidoConCorreo.toLowerCase() === correo))
    },
    tareasFiltradas() {
  ...
  watch: { tareasFiltradas: { handler() { this.syncColumns() }, immediate: true } },
  ...
  methods: {
    ...
    openTask(tarea) { this.editingId = tarea.id; this.readOnly = !this.canEdit(tarea); this.form = { ...tarea }; this.dialog = true },
    syncSharedPerson(id) {
      const persona = this.personal.find(item => item.id === id)
      this.form.compartidoConNombre = persona?.nombres || ''
      this.form.compartidoConCorreo = persona?.correo || ''
    },
    async saveTask() {
