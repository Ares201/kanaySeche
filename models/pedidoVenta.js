...
export function normalizePedidoVenta(data) {
  ...
  return {
    ...
    detalle: detalle.map((row, index) => ({
      ...
      cantidadEnvase: row.cantidadEnvase || '',

      pesoDeclaradoCliente: normalizeNumber(
        row.pesoDeclaradoCliente
      ),
...
export function calculatePesoNeto(
  ...
  return Math.max(
    normalizeNumber(pesoIngreso) -
    normalizeNumber(pesoSalida),
    0
  )
}

export function getNextPedidoVentaCodigo(
  pedidos,
  editingId = null
) {
